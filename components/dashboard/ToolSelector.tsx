...
import { Sliders, Globe, BookOpen, Wand2, Image } from "lucide-react"

interface ToolSelectorProps {
  selectedTool: 'web-search' | 'study' | 'canvas' | 'image-generation' | null
  onToolSelect: (tool: 'web-search' | 'study' | 'canvas' | 'image-generation' | null) => void
  onRemoveTool: () => void
}
      ...
      {selectedTool && (
        <div className="flex items-center space-x-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5">
          ...
          <span className="text-sm font-medium text-blue-700">
            {selectedTool === 'web-search' ? 'Web Search' : selectedTool === 'study' ? 'Study' : selectedTool === 'canvas' ? 'Canvas' : 'Image Generation'}
          </span>
          <button
